...
class TestStatsEnhancedPbpLoader:
    ...
    @responses.activate
    def test_lineup_reconciler_runs_when_enabled(self):
        os.environ["PBPSTATS_USE_STATS_LINEUPS"] = "1"
        try:
            with open(f"{self.data_directory}/pbp/stats_{self.game_id}.json") as f:
                pbp_response = json.loads(f.read())
            base_url = "https://stats.nba.com/stats/playbyplayv2"
            ...
            home_url = furl(shots_base_url).add(home_query_params).url
            away_url = furl(shots_base_url).add(away_query_params).url
            responses.add(responses.GET, home_url, json=home_response, status=200)
            responses.add(responses.GET, away_url, json=away_response, status=200)
            ...
            with open(
                ...
                summary_response = json.loads(f.read())
            summary_base_url = "https://stats.nba.com/stats/boxscoresummaryv2"
            summary_url = furl(summary_base_url).add({"GameId": self.game_id}).url
            responses.add(responses.GET, summary_url, json=summary_response, status=200)

            with open(
                f"{self.data_directory}/period_starters_boxscore_response.json"
            ) as f:
                starters_response = json.loads(f.read())
            boxscore_base_url = "https://stats.nba.com/stats/boxscoretraditionalv2"
            starters_query_params = {
            ...
            with open(
                f"{self.data_directory}/game_details/stats_gamerotation_{self.game_id}.json"
            ) as f:
                rotation_response = json.loads(f.read())
            rotation_base_url = "https://stats.nba.com/stats/gamerotation"
            rotation_params = {
                "GameID": self.game_id,
                "LeagueID": "00",
                "Season": "2016-17",
            ...
            rotation_url = furl(rotation_base_url).add(rotation_params).url
            responses.add(
                responses.GET, rotation_url, json=rotation_response, status=200
            )
            ...
            starters_override = getattr(
                pbp_loader.items[0], "_current_players_override", None
            )
            assert starters_override is not None
            for team_players in starters_override.values():
                assert len(team_players) == 5
        finally:
            os.environ.pop("PBPSTATS_USE_STATS_LINEUPS", None)
