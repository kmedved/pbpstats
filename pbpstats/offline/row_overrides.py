...
def _parse_int(
    value: str, field: str, row_number: int, strict: bool, *, required: bool
) -> int | None:
    if not value:
...
def _validate_parsed_override(row: dict, row_number: int, strict: bool) -> bool:
    ...
    if action in {"move_before", "move_after", "insert_sub_before", "insert_sub_after"}:
        if row.get("anchor_event_num") is None:
            raise ValueError(
                f"Row {row_number} action {action} requires anchor_event_num"
            )
        if int(row["event_num"]) == int(row["anchor_event_num"]):
            raise ValueError(
                f"Row {row_number} action {action} cannot anchor to itself"
            )
...
def load_pbp_row_overrides(
    ...
    *,
    ...
    if strict and missing_columns:
        raise ValueError(
            f"PBP row override catalog missing columns: {sorted(missing_columns)}"
        )
    ...
    for row_number, row in enumerate(df.to_dict(orient="records"), start=2):
        ...
        try:
            ...
            if strict:
                raise ValueError(
                    f"Row {row_number} has invalid game_id: {raw_gid!r}"
                ) from exc
            continue
        event_num = _parse_int(
            raw_event, "event_num", row_number, strict, required=True
        )
        anchor_event_num = _parse_int(
            raw_anchor, "anchor_event_num", row_number, strict, required=False
        )
...
def _infer_sub_description_column(
    ...
    if player_out_team_id is not None and {"PLAYER1_TEAM_ID", "EVENTMSGTYPE"}.issubset(
        df.columns
    ):
...
def _build_synthetic_sub_row(
    ...
    if player_out_id is None or player_in_id is None:
        raise ValueError(
            "Synthetic substitution override requires player_out_id and player_in_id"
        )
    ...
    if not player_out_name or not player_in_name:
        raise ValueError(
            "Synthetic substitution override requires player_out_name and player_in_name"
        )
    ...
    pctimestring = (
        str(override.get("pctimestring", "")).strip()
        or str(anchor_row.get("PCTIMESTRING", "")).strip()
    )
    wctimestring = (
        str(override.get("wctimestring", "")).strip()
        or str(anchor_row.get("WCTIMESTRING", "")).strip()
    )
...
def _single_event_index(
    event_nums: pd.Series,
    event_num: int,
    *,
    context: str,
    strict_lookup: bool,
) -> int | None:
    matches = event_nums.index[event_nums == event_num]
    if len(matches) == 1:
        return int(matches[0])
    if strict_lookup:
        raise ValueError(
            f"{context} expected exactly one row with EVENTNUM {event_num}, "
            f"found {len(matches)}"
        )
    return None
...
def apply_pbp_row_overrides(
    ...
    * ``move_before`` / ``move_after`` -- the row whose ``EVENTNUM`` matches
      ...
      unchanged. Ambiguous or missing lookups are skipped unless
      ``strict_lookup=True``.
    * ``drop`` -- the row whose ``EVENTNUM`` matches ``event_num`` is removed.
      Ambiguous or missing lookups are skipped unless ``strict_lookup=True``.
    * ``insert_sub_before`` / ``insert_sub_after`` -- a synthetic SUB row
    ...
    strict_lookup :
        Raise when an override cannot find exactly one target/anchor row, or
        when an insert action's synthetic ``event_num`` already exists. This is
        intended for committed historic catalogs where silent no-ops would hide
        source drift; ad hoc parser users can keep the default tolerant mode.
    ...
    ValueError
        ...
        unknown action, if a ``move_before`` / ``move_after`` /
        ``insert_sub_*`` override is missing ``anchor_event_num``, or if
        ``strict_lookup=True`` and a referenced row cannot be matched exactly.
    """
    ...
    for override in applicable:
        ...
        if anchor_event_num is not None and int(event_num) == int(anchor_event_num):
            raise ValueError(
                f"{action} override for event {event_num} cannot anchor to itself"
            )

        if action in {"insert_sub_before", "insert_sub_after"}:
            if anchor_event_num is None:
                raise ValueError(
                    f"{action} override for event {event_num} requires anchor_event_num"
                )
            existing_idx = event_nums.index[event_nums == event_num]
            if len(existing_idx) != 0:
                if strict_lookup:
                    raise ValueError(
                        f"{action} override for event {event_num} cannot insert "
                        f"because {len(existing_idx)} row(s) already exist"
                    )
                continue
            for column in (
                ...
                if column not in df.columns:
                    df[column] = ""
            anchor_idx = _single_event_index(
                event_nums,
                anchor_event_num,
                context=f"{action} override anchor for event {event_num}",
            ...
            df = pd.concat(
                [df.iloc[:insert_at], row, df.iloc[insert_at:]], ignore_index=True
            )
            event_nums = pd.to_numeric(df["EVENTNUM"], errors="coerce")
            continue

        row_idx = _single_event_index(
            event_nums,
            event_num,
            context=f"{action} override target",
            strict_lookup=strict_lookup,
        )
        if row_idx is None:
            continue
        ...
        anchor_idx = _single_event_index(
            event_nums,
            anchor_event_num,
            context=f"{action} override anchor after moving event {event_num}",
            strict_lookup=True,
        )
