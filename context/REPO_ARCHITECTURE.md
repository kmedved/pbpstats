...
Architecture sync version: 1.4.0
...
| stats PBP web/file source | v2 cache/endpoint or `endpoint_strategy` = `v3_synthetic` / `auto` | `StatsNbaPbp*Loader` + private v3 synthetic transformer | Returns v2-shaped rows for validated NBA v3 payloads; true v2 cache stays under `/pbp`, raw v3 under `/pbp_v3`, synthetic rows under `/pbp_synthetic_v3`. |
...
- `stats_nba` synthetic v3 PBP must emit the existing playbyplayv2 row contract (`EVENTMSGTYPE`, `EVENTMSGACTIONTYPE`, `PLAYER1_*`, `PLAYER2_*`, `PLAYER3_*`) so the PBP, enhanced PBP, and possession stacks stay unchanged. It is currently enabled only for NBA game ids; WNBA shotchartdetail has the same shot schema, but WNBA playbyplayv3 is not role-complete enough for synthetic v2 PBP parity. (`pbpstats/data_loader/stats_nba/pbp/v3_synthetic.py`, `tests/data_loaders/test_stats_v3_synthetic.py`)
